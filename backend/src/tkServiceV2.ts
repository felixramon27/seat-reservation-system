...
const start = async () => {
  ...
  app.get("/", async () => {
    return {
      service: "TKServiceV2",
      status: "online",
      type: "Microservicio de Tiempo Real",
    };
  });
  ...
  wsApp.register(async function (fastify) {
    ...
    fastify.get("/", { websocket: true } as any, (connection: any, req) => {
      ...
      socket.send(
        JSON.stringify({
          type: "WELCOME",
          message: "Conectado a TKServiceV2 Realtime",
        }),
      );
    });
  });
  ...
  try {
    ...
    changeStream.on("change", (change) => {
      ...
      if (
        ...
        console.log(
          `🔄 Cambio detectado en silla ${docId}: ${document.status}`,
        );
  ...
  setInterval(async () => {
    try {
      ...
      if (result.modifiedCount > 0) {
        console.log(
          `🧹 Mantenimiento: Se liberaron ${result.modifiedCount} sillas por tiempo expirado.`,
        );
      }
    } catch (error) {
