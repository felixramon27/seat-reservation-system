...
import SvgAnalysisPanel from "@/components/svg/SvgAnalysisPanel";
...
export default function Home() {
  ...
  const mapFile = selectedSvg
    ? (selectedSvg.split("/").pop() || "").split("?")[0]
    : "";
  const {
    seats,
    selectSeat,
  ...
  return (
    <main style={{ padding: 40, fontFamily: "Arial, sans-serif" }}>
      ...
      {heldSeats.length > 0 && mode === "client" && (
        ...
        >
          <p style={{ margin: "0 0 10px 0", color: "#856404" }}>
            Tienes <b>{heldSeats.length}</b> asientos reservados temporalmente.
            ¡Confirma tu compra antes de que expire el tiempo!
          </p>
          <button
      ...
      {mode === "admin" && selectedSvg && (
        <SvgAnalysisPanel svgUrl={selectedSvg} />
      )}
