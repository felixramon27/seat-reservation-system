...
export default function SvgSelector({ onSelectSvg, selectedSvg, isAdmin }: Props) {
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`http://localhost:3002/svg/${encodeURIComponent(deleteTarget)}`, {
        method: 'DELETE'
      })
          ...
          {svgs.map(svg => (
            <div key={svg.name} style={{ margin: 6, display: 'flex', gap: 8, alignItems: 'center' }}>
              <button
                ...
                style={{
                  padding: '8px 12px',
                  backgroundColor: selectedSvg === svg.url ? '#28a745' : '#6c757d',
                  color: 'white',
                  border: 'none',
                  borderRadius: 6,
                  cursor: 'pointer'
        ...
        <div style={{ position: 'fixed', left: 0, top: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)' }}>
          <div style={{ background: 'white', padding: 20, borderRadius: 8, minWidth: 300 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
              ...
              <button onClick={handleConfirmDelete} disabled={deleting} style={{ padding: '6px 10px', borderRadius: 6, background: '#dc3545', color: 'white' }}>{deleting ? 'Eliminando...' : 'Eliminar'}</button>
            </div>
